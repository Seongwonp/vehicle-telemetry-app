...
- **2026-09-06부터 format·analyze·test는 CI가 대신 돌린다**(`.github/workflows/ci.yml`).
  SDK가 없는 환경에서 작업하더라도 push하면 결과가 남으므로, "돌리지 못했다"가
  더는 그대로 넘어가지 않는다. 다만 CI가 대체하지 **못하는** 것이 있다 —
  통합 테스트, 골든 스냅샷, 그리고 **실기기 확인(스크롤·터치·키보드·네트워크 재연결)**.
  이 셋은 여전히 사람이 해야 하고, 하지 않았으면 `미검증`으로 적는다.
...
- 연결 중·재연결·오래된 데이터 상태에서 큰 숫자 자리가 비고(`—`), 지난 값이 받은 시각과 함께 작게만 보이는가 — 위젯 테스트는 `dashboard_websocket_test`가 검사한다. 실기기 확인은 별도
- 긴 차량 이름이 잘릴 때 앱바의 전체 이름 버튼으로 전체 이름을 볼 수 있는가(글자 1.5배 포함)
