...
/// 내용의 최대 폭. 넓은 화면에서 한 줄이 얼마나 길어져도 되는가.
///
/// 화면마다 `maxWidth`를 따로 정해 쓰고 있었고, 세어보니 **420 / 480 / 600 / 700 /
/// 800 / 900 / 1100 / 1200 — 8종**이었다. 간격·글자 크기에서 고쳤던 것과 같은
/// "규칙 없음" 문제다. 값 하나하나는 그럴듯해도 화면을 옮길 때마다 내용 폭이
/// 미묘하게 달라진다.
///
/// 기준은 **한 줄에 무엇이 들어가는가**다. 폭 자체가 아니라 내용의 종류로 고른다.
class ContentWidths {
  ContentWidths._();

  /// 480 — 입력 한 열. 로그인, 차량 등록처럼 필드를 세로로 쌓는 화면.
  ///
  /// 폼은 넓다고 좋아지지 않는다. 입력란이 길어지면 라벨과 값 사이 시선 이동만 늘어난다.
  static const double form = 480;

  /// 640 — 읽는 한 열. 설정 목록, 진단 결과처럼 문장이 이어지는 화면.
  ///
  /// 타이포그래피의 일반적인 권장 행 길이(45~75자)를 본문 크기(14~16px)에서
  /// 넘지 않는 폭이다. 이보다 넓으면 줄 끝에서 다음 줄 머리를 찾기 어려워진다.
  static const double reading = 640;

  /// 840 — 카드 한 열. 이상 이력처럼 카드를 세로로 쌓는 목록.
  ///
  /// 카드 안에는 배지·수치가 가로로 놓이므로 본문보다 여유가 필요하지만,
  /// 한 열인 이상 무한정 늘리면 카드 안이 텅 빈다.
  static const double feed = 840;

  /// 1200 — 여러 열. 대시보드 계측 그리드, 차량 목록 그리드, 랜딩.
  ///
  /// 열이 여러 개면 각 열의 폭이 알아서 제한되므로 전체는 넓어도 된다.
