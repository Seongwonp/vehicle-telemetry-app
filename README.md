...
| 대시보드 | STOMP WebSocket 실시간 센서 + 재연결/stale 상태 + 역전·동일 timestamp 방어 + 속도 추이 차트 |
...
WebSocket 재연결·최신 토큰·stale·malformed frame·역전 및 동일 timestamp 재전달,
진단 결과/탭 상태 유지와
인증 초기화·로그인·로그아웃·refresh 실패, 에러/빈 상태를 검증한다. 실제 백엔드
통합 흐름은 `integration_test/app_test.dart`에 있다.

GitHub Actions에서도 `dart format`, `flutter analyze`, `flutter test`를 같은 순서로
실행한다. 백엔드와 기기가 필요한 통합 테스트, 플랫폼별 골든 스냅샷, 실기기 조작은
기본 CI 범위가 아니므로 별도로 검증한다.
